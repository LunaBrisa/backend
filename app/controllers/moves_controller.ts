...
export default class MovesController {
  async store({ request, params, auth, response }: HttpContext) {
    try {
      ...
      // ✅ VERIFICAR GANADOR SOLO SI ES HIT Y EL JUEGO AÚN ESTÁ ACTIVO
      if (result === 'hit' && game.status === 'active' && !game.winner) {
        // ✅ CONTAR HITS DESPUÉS DE CREAR EL MOVIMIENTO
        ...
        const totalHits = parseInt(hitsCount[0].$extras.total)

        logger.info('Conteo de hits', {
          player_id: auth.user!.id,
          total_hits: totalHits,
          game_id: game.id,
        })

        // ✅ FINALIZAR JUEGO SI ALCANZA 15 HITS
        if (totalHits >= 15) {
          await game
            .merge({
              status: 'finished',
          ...
          logger.info('🏆 JUEGO FINALIZADO - GANADOR DETECTADO', {
            winner: auth.user!.id,
            total_hits: totalHits,
            game_id: game.id,
          })
  ...
  async poll({ request, params, auth, response }: HttpContext) {
    try {
      ...
      // ✅ VERIFICAR INACTIVIDAD ANTES DE PROCESAR
